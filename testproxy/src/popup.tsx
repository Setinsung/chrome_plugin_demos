...
const IndexPopup = () => {
  ...
  const getProxyData = async () => {
    const proxyData = await sendToBackground({
      name: "getProxyData"
    });
    console.log('proxyData', proxyData);
    setServer({ host: proxyData.host, port: proxyData.port });
          ...
          <div className="mb-3">
            ...
            <div className="p-1 bg-purple-100 shadow-md rounded flex justify-between items-center">
              <div>
                ...
                <input value={server.port} onChange={e =>setServer({ ...server, port: e.target.value })} id="port" type="text" placeholder="8080" className='text-xs py-[0.1rem] px-[0.25rem] w-[2.2rem] rounded-full outline-none' />
              </div>
